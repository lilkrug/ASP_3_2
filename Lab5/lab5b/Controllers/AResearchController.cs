...
namespace lab5b.Controllers
{
    public class AResearchController : Controller
    {
        ...
        public class AAFilter : FilterAttribute, IActionFilter  //FilterAttribute - это атрибут в языке C#, который используется в ASP.NET для реализации фильтров действий
        {
            //метод, который вызывается перед выполнением действия. Он может использоваться для обработки запроса
            //перед выполнением действия или для прерывания выполнения действия, если необходимо.
            public void OnActionExecuting(ActionExecutingContext filterContext) 
            {
                filterContext.HttpContext.Response.Write("AA:Вызов перед вызовом метода действия\n");
            }
            //метод, который вызывается после выполнения действия. Он может использоваться для обработки ответа
            //после выполнения действия или для модификации результата действия.
